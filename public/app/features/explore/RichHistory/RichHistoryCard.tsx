...
export function RichHistoryCard(props: Props) {
  ...
  const onRunQuery = async () => {
    ...
    if (differentDataSource) {
      changeDatasource(exploreId, query.datasourceUid);
    }
    ...
    const queriesText = (
      await Promise.all(
        query.queries.map(async (q) => {
          ...
          if (dsInstance instanceof MixedDatasource) {
  ...
  return (
    <div className={styles.queryCard}>
      {/* TODO: if loading retun a loader */}
      {loading}
      ...
      <div className={cx(styles.cardRow)}>
        <div className={styles.queryContainer}>
          {query.queries.map((q, i) => {
            return <Query query={q} key={`${q}-${i}`} showDsInfo={dsInstance instanceof MixedDatasource} />;
          })}
...
function DatasourceInfo({ dsApi, size }: { dsApi?: DataSourceApi; size: 'sm' | 'md' }) {
  ...
  return (
    <div className={styles}>
      <img src={dsApi?.meta.info.logos.small || 'public/img/icn-datasource.svg'} alt="" />
      <div>{dsApi?.name || 'Missing data source'}</div>
